...
class BaseFast(nn.Module):
    def __init__(self, configs, word_vectors):
        ...
        self.label2d_mask = generate_2dmask(max_pos_len).to(configs.device)
        ...
        # self.C_GCN = C_GCN(3152, in_channel=300, t=0.3, embed_size=64, 
        ...
        #                     com_path='/storage/ymyang/4_FastVMR/CCA/acnet_concept/acnet_com_graph.pkl')
        # self.V_TransformerLayer = nn.TransformerEncoderLayer(3216, 8)

        # self.fusion_layer = nn.TransformerEncoderLayer(64, 8)
        # self.fusion_layer = torch.nn.MultiheadAttention(D, D, dropout=droprate) 
    ...
    def forward(self, word_ids, char_ids, vfeat_in, vmask, tmask):
        ...
        # vfeat = torch.cat([vfeat.permute(0, 2, 1), concept_basis.unsqueeze(0).repeat(vfeat.size(0), 1, 1).permute(0, 2, 1)], dim=2)
        # vfeat = self.V_TransformerLayer(vfeat)[:, :, :64].permute(0, 2, 1)
        ...
        f_fusion = vfeat * f_tfeat[:, None, :]
        # f_fusion, _ = self.fusion_layer(vfeat, f_fusion, f_fusion)

        slogits, elogits = self.predictor(f_fusion, vmask)

        # slogits = torch.sigmoid(slogits)
        # elogits = torch.sigmoid(elogits)
        logit2ds = torch.matmul(slogits.unsqueeze(2), elogits.unsqueeze(1)) * self.label2d_mask
        ...
        res = {
            ...
            "elogits": elogits,
            "vmask" : vmask,
            "label2d_mask" : self.label2d_mask,
            "logit2ds": logit2ds,
...
def lossfun_softloc2(logit, label, vmask, T):
    from models.layers import mask_logits
    label = F.interpolate(label.unsqueeze(0), logit.shape[1], mode='linear', align_corners=True).squeeze(0)
    vmask = vmask.bool()
    # logit = logit.masked_select(vmask)
    # label = label.masked_select(vmask)
    logit = logit * vmask
    label = label * vmask
    # logit = F.softmax(F.normalize(logit, p=2, dim=1) / T, dim=-1) 
    # label = F.softmax(F.normalize(label, p=2, dim=1) / T, dim=-1) 

    logit = F.normalize(logit, p=2, dim=1)
    logit_sim = torch.matmul(logit, logit.T)
    logit_sim = torch.softmax(logit_sim / T, dim=-1)

    label = F.normalize(label, p=2, dim=1)
    label_sim = torch.matmul(label, label.T)
    label_sim = torch.softmax(label_sim / T, dim=-1)
    sloss = F.kl_div(logit_sim.log(), label_sim, reduction='sum')
    return sloss



def lossfun_softloc3(logit, label, vmask, T):
    from models.layers import mask_logits
    label = F.interpolate(label.unsqueeze(0), logit.shape[1], mode='linear').squeeze(0)
    vmask = vmask.bool()
    ...
    sloss = F.kl_div(logit.log(), label, reduction='sum')
    return sloss
...
def train_engine_BaseFast(model, data, configs):
    ...
    softloc_loss = lossfun_softloc2(slogits, label1d_model1s[:,0,:], data['vmasks'], 3) \
                 + lossfun_softloc2(elogits, label1d_model1s[:,1,:], data['vmasks'], 3) 
    # loc2d_loss = lossfun_loc2d(output["logit2ds"], data["label2ds"], output['label2d_mask'])
    ...
    loss = loc_loss + 1.0 * align_loss + 1.0 *softloc_loss # + loc2d_loss# + 1.0 *softloc_loss# + loc2d_loss # + 0.5*softloc_loss + loc2d_loss
    # loss = loc_loss
    return loss, output
...
# def infer_BaseFast(output, configs):
#     scores2d, label2d_mask, vmask = output["logit2Ds"], output['label2d_mask'], output['vmask']
#     res = infer_basic2d(scores2d, label2d_mask, vmask)
#     return res