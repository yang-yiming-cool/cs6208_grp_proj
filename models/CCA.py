...
from models.layers import  SeqPANPredictor
...
class CCA(nn.Module):
    def __init__(self, cfg, word_vectors):
        ...
        # BaseFast
        self.predictor = SeqPANPredictor(cfg)
    ...
    def forward(self, words_ids, tmask, vfeat_in, vmask, concept_input_embs):
        ...
        # BaseFast
        v2t_map2d = feats * queries[:, :, None]
        slogits, elogits = self.predictor(v2t_map2d, vmask)
...
def train_engine_CCA(model, data, configs):
    ...
    loc_loss = lossfun_loc(output["slogits"], output["elogits"], label1ds[:, 0, :], label1ds[:, 1, :], data['vmasks'])
    loss = loc_loss
